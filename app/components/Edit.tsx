...
export default function Edit({ product, onclose }) {
  const [id, setId] = useState(product.id);
  const [name, setName] = useState(product.attributes.Name);
  const [price, setPrice] = useState(product.attributes.Price);
  const [description, setDescription] = useState(product.attributes.Desc);
  const [photo, setPhoto] = useState(product.attributes.img.data.attributes.url);
  ...
  // onClick={handleCloseOnOverlayClick}
    >
      ...
      <div className="mt-0" >
        <button onClick={handleCloseOnOverlayClick}>Click here to close</button>
      </div>  
